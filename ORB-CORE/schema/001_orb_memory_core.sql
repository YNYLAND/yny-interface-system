...
-- 1) Semantic sessions: parallel goal/process lines inside any technical chat/session.
...
-- 2) Checks / open loops inside a semantic session.
...
-- 3) Link semantic events to semantic sessions.
-- Existing memory_events remains the event stream / 'notes in the margins'.
...
-- Optional event semantics. Existing raw_event remains authoritative for flexible payloads.
alter table public.memory_events
...
-- Confidence is intentionally nullable: observed/confirmed events need not be inferred.
do $$
begin
  if not exists (
    select 1
...
-- 4) Convenience view: Actor achievements = results of completed/closed semantic sessions.
...
-- 5) updated_at helper (safe to replace).
